...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        else
        {
            ...
            $products =  Product::select('products.*')
                        ...
                        ->leftjoin('products AS products_dd', 'products_dd.dependent_dropdown_id', '=', 'products.id')
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                $nestedData['par_quantity'] = $product->par_quantity;
                if($product->unit_id)
                    $nestedData['unit'] = $product->unit->unit_name;
                else
                    $nestedData['unit'] = 'N/A';
                // $ prefix added to cost and price by usha on 6/4/2023
                $nestedData['price'] = '$'.$product->price;
                $nestedData['cost'] = '$'.$product->cost;

                // $nestedData['dependent_dropdown_id'] = ;
                if ($product->dependent_dropdown_id) {
                    $prodData = Product::where('id', $product->dependent_dropdown_id)->select('name')->get();
                    $nestedData['dependent_dropdown_id'] = $prodData[0]->name;
                }
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            ...
            return view('product.create',compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list', 'lims_product_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->dependent_dropdown_id) {
            $data['dependent_dropdown_id'] = $request->dependent_dropdown_id;
        }
        else{
            $data['dependent_dropdown_id'] = null;
        }
        
        /*$data['boxes'] = $request->boxes;
        $data['carton'] = $request->carton;
        $data['pallete'] = $request->pallete;*/
        ...
        try {
            ...
            $prodData = Product::latest()->get();
            Log::info('Product Passed');
            $purchase = Purchase::create([
                'reference_no'=> 'po-' . date("Ymd") . '-'. date("his"),
                'user_id'=> Auth::id(),
                'warehouse_id'=> 11,
                'supplier_id'=> 6,
                'item'=> 1,
                ...
                'total_discount'=> 0.0,
                'total_tax'=> 0.0,
                'total_cost'=> $prodData[0]->cost,
                'order_tax_rate'=> 0.0,
                'order_tax'=> 0.0,
                'order_discount'=> null,
                'shipping_cost'=> null,
                'grand_total'=> $prodData[0]->cost,
                'paid_amount'=> 0.0,
                'status'=> 3,
                'payment_status'=> 1,
                'document'=> null,
            ...
            $purchase_last_id = Purchase::max('id');
            $product_last_id = Product::max('id');

            // Log::info(Purchase::max('id'));

            ProductPurchase::create([
                'purchase_id'=> $purchase_last_id,
            ...
            Log::info('Product purchase created');
          }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            $lims_product_list = Product::where('is_active', true)->get();
            $dependent_product_of_id = Product::where([['id', $id], ['is_active', true]])->select('dependent_dropdown_id')->get();

            return view('product.edit',compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'lims_product_list', 'dependent_product_of_id'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function updateProduct(Request $request)
    {
        ...
        //else {
            ...
            $data['dependent_product_id'] = $request->dependent_dropdown_id;
	...
	public function printRackBarcode()
    {
        $lims_product_list_without_variant = $this->productWithoutVariant();
        $lims_product_list_with_variant = $this->productWithVariant();
        return view('product.print_rack_barcode', compact('lims_product_list_without_variant', 'lims_product_list_with_variant'));
    }
    ...
    {   
        ...
        while($columns=fgetcsv($file))
        {
           ...
           //dd($data);  
           if($data['brandmanufacturer'] != 'N/A' && $data['brandmanufacturer'] != ''){
                $lims_brand_data = Brand::firstOrCreate(['title' => $data['brandmanufacturer'], 'is_active' => true]);
           ...
           $product = Product::firstOrNew([ 'name'=>$data['productname'], 'is_active'=>true ]);
            if($data['image'])
                $product->image = $data['image'];
            else
                $product->image = 'zummXD2dvAtI.png';

           $product->name = htmlspecialchars(trim($data['productname']));
           ...
           $product->qty = 0;
           $product->par_quantity = (int) $data['parquantity'] ?? 0;
